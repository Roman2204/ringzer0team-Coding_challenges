...
page_task = my_session.get('http://ringzer0team.com/challenges/14')
# print page_task.content

# Carve a message for hashing
message_bin = re.findall(re.compile('----- BEGIN MESSAGE -----<br />[\r\n\s]*(.+?)[\r\n\s]*<br />'), page_task.text)
message_ascii = binascii.unhexlify('%x' % int(message_bin[0], 2))
# print message_bin[0]
# print message_ascii
# Get a hash from message
message_hash = hashlib.sha512(message_ascii)
# print message_hash.hexdigest()

# Send our hash to server
page_answer = my_session.get('http://ringzer0team.com/challenges/14/' + message_hash.hexdigest())
